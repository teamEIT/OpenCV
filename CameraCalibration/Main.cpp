...
void fprintfVectorMat(vector< Mat > matrix, string name) {
	FILE* file;
		...
		int i, j;
		printf("%s size %d %d \n", name.c_str(), matrix[0].cols, matrix[0].rows);

		for(i = 0; i < matrix[i].rows; i++) {
			for(j = 0; j < matrix[j].cols; j++) {
				fprintf(file, "%lf", matrix[i].at<double>(i, j)); // Returns a reference to the specified array element
			}
			fprintf(file, "\n");
		}
		fclose(file);
...
void fprintf2Point(vector< vector< Point2f > > Points, string name) {
	...
	int i, j;
	for(i = 0; i < Points[i].size(); i++) {
		for(j = 0; j < Points[i].size(); j++) {
			fprintf(file, "%lf %lf \n", Points[i][j].x, Points[i][j].y); // Returns a reference to the specified array element
		}
		fprintf(file, "\n");
	}
	fclose(file);
...
void fprintf3Point(vector< vector< Point3f > > Points, string name) {
	FILE* file;
		...
		for(i = 0; i < Points[i].size(); i++) {
			for(j = 0; j < Points[i].size(); j++) {
				fprintf(file, "%lf %lf %lf \n", Points[i][j].x, Points[i][j].y, Points[i][j].z); // Returns a reference to the specified array element
			}
			fprintf(file, "\n");
		}
		fclose(file);
	...
	for(int i=0; i< nBoards; ++i)
	 {
	  ...
	  bool sCorner = findChessboardCorners(gray, Size(boardW, boardH), corners);
	  ...
	  if(sCorner)
	  {
	   ...
	   //draw corner
	   drawChessboardCorners(img, Size(boardW, boardH), corners, sCorner);
	   if(corners.size() == boardW*boardH)
	   {
	    ...
	    for(int j=0; j< corners.size(); ++j)
	    {
	     ...
	     tObjPT.x = j%boardW*measure;
	     tObjPT.y = j/boardW*measure;
	     tObjPT.z = 0;
